...
export const fetchOrders = createAsyncThunk(
  'orders/fetchOrders',
  async (params: OrderFilters & OrderPagination, { rejectWithValue }) => {
    try {
...
export const updateOrderStatus = createAsyncThunk(
  'orders/updateStatus',
  async ({ id, status }: UpdateOrderStatusPayload, { rejectWithValue }) => {
    try {
      await new Promise(resolve => setTimeout(resolve, 1000));
...
export const assignDeliveryAgent = createAsyncThunk(
  'orders/assignAgent',
  async ({ orderId, agentId }: AssignDeliveryAgentPayload, { rejectWithValue }) => {
    try {
      await new Promise(resolve => setTimeout(resolve, 1000));
